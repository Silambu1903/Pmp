...
class DailyTask extends ConsumerStatefulWidget {
  ProjectManagementProcessProjectAssignTeam? data;
  TaskAssignToUserList? taskIdList;

  DailyTask({Key? key, this.data, this.taskIdList}) : super(key: key);

  @override
  _DailyTaskState createState() => _DailyTaskState();
}

class _DailyTaskState extends ConsumerState<DailyTask> {
  ...
  windowsUi(BuildContext context) {
    return ref.watch(dailyTaskListNotifier(widget.taskIdList!.id!)).when(
        data: (data) {
      spentHourTotal = 0;
      if (widget.data!.projectCreation!.status == 'Open') {
        if (data.isNotEmpty) {
          ref
                    ...
                    return AlertDialog(
                      ...
                      title: Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                             Text(widget.taskIdList!.taskCreation!.taskName.toString()),
                            IconButton(
                              onPressed: () {
                                Navigator.pop(context);
                              },
                              ...
                              if (validateField()) {
                                ref.read(dailyTaskNotifier.notifier).dailyTask(
                                    ProjectManagementProcessDailyReport(
                                        ...
                                        taskAssignId: widget.taskIdList!.id),widget.data!.id!);
  ...
  bool validateField() {
    if (taskName.text.isEmpty) {
      ...
      displayErrorMessages('Enter the Spent Hours');
      return false;
    } else if (remarks.text.isEmpty) {
      displayErrorMessages('Enter your Remarks');
      return false;
    } else if (spentHours.text.isEmpty) {
      displayErrorMessages('Enter your Remarks');
      return false;
